 ...
 * - [PurchaseOrderItemsCreateFragment]: to create or edit a(n) PurchaseOrderItem,
 * - [PurchaseOrderItemsDetailFragment]: to display the details of a(n) PurchaseOrderItem,
 * - [PurchaseOrderItemsListFragment]: to list PurchaseOrderItems.
 *
 ...
 * what to do.
 */
class PurchaseOrderItemsActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<PurchaseOrderItem> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, PurchaseOrderItemsListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: PurchaseOrderItem?) {
        when (evt) {
    ...
    private fun onItemClicked(entity: PurchaseOrderItem?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, PurchaseOrderItemsDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, PurchaseOrderItemsDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: PurchaseOrderItem?) {
        val updateFragment = PurchaseOrderItemsCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as PurchaseOrderItemsActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[PurchaseOrderItemViewModel::class.java]

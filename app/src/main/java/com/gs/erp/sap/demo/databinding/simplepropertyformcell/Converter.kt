...
import com.sap.cloud.android.odata.com_sap_mbtepmdemoservice.Gender
import com.sap.cloud.android.odata.com_sap_mbtepmdemoservice.GenderConvert
...
import com.sap.cloud.android.odata.com_sap_mbtepmdemoservice.Com_sap_mbtepmdemoServiceMetadata.EnumTypes
...
object Converter {
    ...
    /*
     * For Enum Type Gender specified in metadata document
     * Handles two way data binding to and from Enum Type Gender
     */
    @InverseMethod("toGender")
    @JvmStatic
    @Suppress("UNUSED_PARAMETER")
    fun toString(cell: SimplePropertyFormCell, oldValue: Gender?, value: Gender?): String {
        if (value == null) {
    ...
    @JvmStatic
    fun toGender(cell: SimplePropertyFormCell, oldValue: Gender?, value: String): Gender? {
        cell.isErrorEnabled = false
        if (value.isEmpty()) {
            return oldValue
        }

        val enumValueList: EnumValueList = EnumTypes.gender.memberList
        for (enumValue: EnumValue in enumValueList) {
            if (enumValue.name.equals(value, true)) {
                return GenderConvert.fromRequiredEnumValue(enumValue)
            }
        }
        cell.isErrorEnabled = true
        cell.error = cell.resources.getString(R.string.format_error)

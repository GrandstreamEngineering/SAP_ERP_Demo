...
import com.gs.erp.sap.demo.databinding.FragmentCustomersDetailBinding
 ...
 */
class CustomersDetailFragment : InterfacedFragment<Customer, FragmentCustomersDetailBinding>() {
    ...
    override fun initBinding(inflater: LayoutInflater, container: ViewGroup?) = FragmentCustomersDetailBinding.inflate(inflater, container, false)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        activity?.let {
            ...
            viewModel.selectedEntity.observe(viewLifecycleOwner) { entity ->
                ...
                fragmentBinding.customer = entity
                setupObjectHeader()
            }
        }
    }
     ...
     */
    private fun setDetailImage(objectHeader: ObjectHeader, customerEntity: Customer) {
        if (customerEntity.getOptionalValue(Customer.city) != null && !customerEntity.getOptionalValue(Customer.city).toString().isEmpty()) {
            objectHeader.detailImageCharacter = customerEntity.getOptionalValue(Customer.city).toString().substring(0, 1)
        } else {
    ...
    /**
     * Setup ObjectHeader with an instance of customerEntity
     */
    private fun setupObjectHeader() {
        val secondToolbar = currentActivity.findViewById<Toolbar>(R.id.secondaryToolbar)
        if (secondToolbar != null) {
            secondToolbar.title = customerEntity.entityType.localName
        } else {
            currentActivity.title = customerEntity.entityType.localName
        }
        ...
        objectHeader?.let {
            it.apply {
                ...
                subheadline = EntityKeyUtil.getOptionalEntityKey(customerEntity)
            ...
            setDetailImage(it, customerEntity)

 ...
 * - [ProductCategoriesCreateFragment]: to create or edit a(n) ProductCategory,
 * - [ProductCategoriesDetailFragment]: to display the details of a(n) ProductCategory,
 * - [ProductCategoriesListFragment]: to list ProductCategories.
 *
 ...
 * what to do.
 */
class ProductCategoriesActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<ProductCategory> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, ProductCategoriesListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: ProductCategory?) {
        when (evt) {
    ...
    private fun onItemClicked(entity: ProductCategory?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, ProductCategoriesDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, ProductCategoriesDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: ProductCategory?) {
        val updateFragment = ProductCategoriesCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as ProductCategoriesActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[ProductCategoryViewModel::class.java]

 ...
 * - [SalesOrderItemsCreateFragment]: to create or edit a(n) SalesOrderItem,
 * - [SalesOrderItemsDetailFragment]: to display the details of a(n) SalesOrderItem,
 * - [SalesOrderItemsListFragment]: to list SalesOrderItems.
 *
 ...
 * what to do.
 */
class SalesOrderItemsActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<SalesOrderItem> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, SalesOrderItemsListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: SalesOrderItem?) {
        when (evt) {
    ...
    private fun onItemClicked(entity: SalesOrderItem?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, SalesOrderItemsDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, SalesOrderItemsDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: SalesOrderItem?) {
        val updateFragment = SalesOrderItemsCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as SalesOrderItemsActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[SalesOrderItemViewModel::class.java]

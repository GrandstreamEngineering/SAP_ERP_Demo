 ...
 * - [PurchaseOrderHeadersCreateFragment]: to create or edit a(n) PurchaseOrderHeader,
 * - [PurchaseOrderHeadersDetailFragment]: to display the details of a(n) PurchaseOrderHeader,
 * - [PurchaseOrderHeadersListFragment]: to list PurchaseOrderHeaders.
 *
 ...
 * what to do.
 */
class PurchaseOrderHeadersActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<PurchaseOrderHeader> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, PurchaseOrderHeadersListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: PurchaseOrderHeader?) {
        when (evt) {
    ...
    private fun onItemClicked(entity: PurchaseOrderHeader?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, PurchaseOrderHeadersDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, PurchaseOrderHeadersDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: PurchaseOrderHeader?) {
        val updateFragment = PurchaseOrderHeadersCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as PurchaseOrderHeadersActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[PurchaseOrderHeaderViewModel::class.java]

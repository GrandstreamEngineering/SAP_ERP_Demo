...
import com.gs.erp.sap.demo.databinding.FragmentSalesorderitemsCreateBinding
 ...
 */
class SalesOrderItemsCreateFragment : InterfacedFragment<SalesOrderItem, FragmentSalesorderitemsCreateBinding>() {
    ...
    private lateinit var salesOrderItemEntityCopy: SalesOrderItem

    /** Indicate what operation to be performed */
    private lateinit var operation: String

    /** salesOrderItemEntity ViewModel */
    ...
    private val isSalesOrderItemValid: Boolean
        get() {
            var isValid = true
            fragmentBinding.createUpdateSalesorderitem.let { linearLayout ->
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        activity?.let {
            (it as SalesOrderItemsActivity).isNavigationDisabled = true
            ...
            salesOrderItemEntity = if (operation == UIConstants.OP_CREATE) {
            ...
            val workingCopy = when{ (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) -> {
                    savedInstanceState?.getParcelable<SalesOrderItem>(KEY_WORKING_COPY, SalesOrderItem::class.java)
                } else -> @Suppress("DEPRECATION") savedInstanceState?.getParcelable<SalesOrderItem>(KEY_WORKING_COPY)
            }

            if (workingCopy == null) {
                salesOrderItemEntityCopy = salesOrderItemEntity.copy()
                salesOrderItemEntityCopy.entityTag = salesOrderItemEntity.entityTag
                salesOrderItemEntityCopy.oldEntity = salesOrderItemEntity
                salesOrderItemEntityCopy.editLink = salesOrderItemEntity.editLink
                salesOrderItemEntityCopy.dataPath = salesOrderItemEntity.dataPath
                salesOrderItemEntityCopy.isContained = salesOrderItemEntity.isContained
            } else {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        fragmentBinding.salesOrderItem = salesOrderItemEntityCopy
        return fragmentBinding.root
    }

    override fun initBinding(inflater: LayoutInflater, container: ViewGroup?) = FragmentSalesorderitemsCreateBinding.inflate(inflater, container, false)
    ...
    private fun onSaveItem(): Boolean {
        ...
        (currentActivity as SalesOrderItemsActivity).isNavigationDisabled = false
        progressBar?.visibility = View.VISIBLE
        when (operation) {
            UIConstants.OP_CREATE -> {
                viewModel.create(salesOrderItemEntityCopy)
            }
            UIConstants.OP_UPDATE -> viewModel.update(salesOrderItemEntityCopy)
        }
        return true
    }
    ...
    private fun createSalesOrderItem(): SalesOrderItem {
        val entity = SalesOrderItem(true)
        return entity
    }
    ...
    private fun onComplete(result: OperationResult<SalesOrderItem>) {
        ...
        if (result.error != null) {
            (currentActivity as SalesOrderItemsActivity).isNavigationDisabled = true
            handleError(result)
        } else {
            if (operation == UIConstants.OP_UPDATE && !currentActivity.resources.getBoolean(R.bool.two_pane)) {
                viewModel.selectedEntity.value = salesOrderItemEntityCopy
            }
            (currentActivity as SalesOrderItemsActivity).onBackPressedDispatcher.onBackPressed()
        }
    }

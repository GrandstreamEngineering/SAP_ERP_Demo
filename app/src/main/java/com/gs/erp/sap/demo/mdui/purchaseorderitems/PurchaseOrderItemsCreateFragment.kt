...
import com.gs.erp.sap.demo.databinding.FragmentPurchaseorderitemsCreateBinding
 ...
 */
class PurchaseOrderItemsCreateFragment : InterfacedFragment<PurchaseOrderItem, FragmentPurchaseorderitemsCreateBinding>() {
    ...
    private lateinit var purchaseOrderItemEntityCopy: PurchaseOrderItem

    /** Indicate what operation to be performed */
    private lateinit var operation: String

    /** purchaseOrderItemEntity ViewModel */
    ...
    private val isPurchaseOrderItemValid: Boolean
        get() {
            var isValid = true
            fragmentBinding.createUpdatePurchaseorderitem.let { linearLayout ->
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        activity?.let {
            (it as PurchaseOrderItemsActivity).isNavigationDisabled = true
            ...
            purchaseOrderItemEntity = if (operation == UIConstants.OP_CREATE) {
            ...
            val workingCopy = when{ (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) -> {
                    savedInstanceState?.getParcelable<PurchaseOrderItem>(KEY_WORKING_COPY, PurchaseOrderItem::class.java)
                } else -> @Suppress("DEPRECATION") savedInstanceState?.getParcelable<PurchaseOrderItem>(KEY_WORKING_COPY)
            }

            if (workingCopy == null) {
                purchaseOrderItemEntityCopy = purchaseOrderItemEntity.copy()
                purchaseOrderItemEntityCopy.entityTag = purchaseOrderItemEntity.entityTag
                purchaseOrderItemEntityCopy.oldEntity = purchaseOrderItemEntity
                purchaseOrderItemEntityCopy.editLink = purchaseOrderItemEntity.editLink
                purchaseOrderItemEntityCopy.dataPath = purchaseOrderItemEntity.dataPath
                purchaseOrderItemEntityCopy.isContained = purchaseOrderItemEntity.isContained
            } else {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        fragmentBinding.purchaseOrderItem = purchaseOrderItemEntityCopy
        return fragmentBinding.root
    }

    override fun initBinding(inflater: LayoutInflater, container: ViewGroup?) = FragmentPurchaseorderitemsCreateBinding.inflate(inflater, container, false)
    ...
    private fun onSaveItem(): Boolean {
        ...
        (currentActivity as PurchaseOrderItemsActivity).isNavigationDisabled = false
        progressBar?.visibility = View.VISIBLE
        when (operation) {
            UIConstants.OP_CREATE -> {
                viewModel.create(purchaseOrderItemEntityCopy)
            }
            UIConstants.OP_UPDATE -> viewModel.update(purchaseOrderItemEntityCopy)
        }
        return true
    }
    ...
    private fun createPurchaseOrderItem(): PurchaseOrderItem {
        val entity = PurchaseOrderItem(true)
        return entity
    }
    ...
    private fun onComplete(result: OperationResult<PurchaseOrderItem>) {
        ...
        if (result.error != null) {
            (currentActivity as PurchaseOrderItemsActivity).isNavigationDisabled = true
            handleError(result)
        } else {
            if (operation == UIConstants.OP_UPDATE && !currentActivity.resources.getBoolean(R.bool.two_pane)) {
                viewModel.selectedEntity.value = purchaseOrderItemEntityCopy
            }
            (currentActivity as PurchaseOrderItemsActivity).onBackPressedDispatcher.onBackPressed()
        }
    }
